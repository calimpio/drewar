...
export namespace Specificator {
    ...
    class Comparation implements IBoolComparator {
        ...
        public static on(field: string | number | symbol, comparator: ComparatorEnum, value: string): IBoolComparator {
                        ...
                        if (field && comparator && (operator || operator === "") && (data || data === "")) {
                            ...
                            if (spec.comparation) {
                                ...
                                if (spec.opertator == BoolOperatorEnum.and || spec.opertator == BoolOperatorEnum.or) {
                                    ...
                                    let aux = spec.top;
                                    if (!aux) throw new Exception.NotMatchSpecificator(`specification with error ${specStr} near to ${spec.comparation.field.toString()} in operator ${spec.opertator}.`)
    ...
    export function equal<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.eq, value)
    }

    export function no<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.ne, value)
    }

    export function lessTo<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.lt, value)
    }

    export function greatTo<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.gt, value)
    }

    export function greatEqualTo<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.gte, value)
    }

    export function lessEqualTo<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.lte, value)
    }

    export function statsWith<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.startWith, value)
    }

    export function endsWith<T>(field: keyof T, value: any) {
        return Comparation.on(field, ComparatorEnum.endsWith, value)
    }

    export function similiarTo<T>(field: keyof T, value: any) {
